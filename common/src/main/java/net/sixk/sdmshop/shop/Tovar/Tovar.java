...
public class Tovar {
    ...
    public void deserialize(KeyData data, HolderLookup.Provider provider) {
        ...
        if(currency == null || currency.isEmpty()) currency = "sdmcoin";
        if(EconomyAPI.getAllCurrency().value.currencies.stream().filter(s ->s.getName().equals(currency)).findAny().isEmpty()) currency = "sdmcoin";
