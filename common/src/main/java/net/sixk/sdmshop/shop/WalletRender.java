...
public class WalletRender extends Panel {
    ...
    public void addWidgets() {
        ...
        if(SDMShop.isEditMode()) {
            add(delete = new SimpleButton(this, Component.translatable("sdm_shop.delete"), Icons.REMOVE, ((simpleButton, mouseButton) -> {
                EconomyAPI.deleteCurrencyOnClient(currency.currency);
                try {
                    Thread.sleep(300);
                ...
                TovarList.CLIENT.tovarList.forEach(s ->{
                        if(s.currency.equals(currency.currency.getName())) s.currency = "sdmcoin";
                    });
    ...
    public void alignWidgets() {
        ...
        if(i.equals("sdm_shop.currency." + currency.currency.getName())) i = currency.currency.getName();
